...
public:
    ...
    void addEdge(int i, int j, NodeWeight w);
...
// to be added.
{
    //cout << "start at " << i << " finish at " << j << "Numnodes "<<numNodes()<<endl;
...
void graph::setNodeParent(int node, int parent)
{
    if (node < 0 || node >= numNodes() || parent < 0 || node >= numNodes())
    {
        throw rangeError("Bad value in graph::setNodeParent");
    }
...
int graph::getNodeParent(int node)
{
    if (node < 0 || node >= numNodes())
    {
        throw rangeError("Bad value in graph::getNodeParent");
    }
...
const int graph::numNodes() const
// Return the number of nodes.
{
    return nodes.size();
}
int graph::nonConstNumNodes()
{
    return nodes.size();
}
